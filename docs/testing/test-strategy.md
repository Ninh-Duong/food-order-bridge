...
- **Admin Shell & Resiliency**:
  ...
  - `test/admin-runtime.test.js`: Mô phỏng DOM runtime thật qua VM, kiểm tra escapeHTML XSS, showDashboard, initAuth fail-safe.
- **Staff Management & Granular Permissions**:
  ...
  - `test/staff-permission-modal.test.js`: Kiểm tra DOM hierarchy (modal nằm ngoài invoice overlay), modal lifecycle, phím Escape và PUT payload.
